...
import { clamp, sumPixel, gaussianVector } from './Utils';
    ...
    bicubicInterpolation(k) {
        ...
        return this._interpolation(k, (x, y, oldX, oldY, tmp) => {
            ...
            if (x1 === x2 || y1 === y2) {
                ...
                for (let i = 0; i < this.channelsNumber; i++) {
                    const col0 = cubic(p00[i], p10[i], p20[i], p30[i], xFract);
                    const col1 = cubic(p01[i], p11[i], p21[i], p31[i], xFract);
                    const col2 = cubic(p02[i], p12[i], p22[i], p32[i], xFract);
                    const col3 = cubic(p03[i], p13[i], p23[i], p33[i], xFract);
                    const value = cubic(col0, col1, col2, col3, yFract);
